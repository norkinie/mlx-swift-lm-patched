...
public struct LFM2VLProcessor: UserInputProcessor {
    ...
    func splitIntoPatchesAndPreprocess(
        ...
        // below — vastly exceeds any sane budget (measured: 17,920 tokens,
        // reproducibly crashing generate() with a token/feature count
        // mismatch, SIGTRAP, on the Fidarix test device).
        //
        // First pass (superseded, kept as history): shrunk BOTH axes down
        // to config.minTiles whenever the combined token count exceeded
        // config.maxImageTokens (256, from processor_config.json). That
        // stopped the crash but re-squashed the aspect ratio into a
        // near-square shape (2×2 tiles → 1024×1024 resize for this photo,
        // 5.7% of the original pixel count) — the same class of distortion
        // this whole model switch was meant to avoid (see the Gemma-4-E2B
        // 224×224 anisotropic-squash comment history in
        // ModelDownloadService.ts). config.maxImageTokens/minImageTokens
        // turned out to target a different (non-tiled) code path in the
        // reference HF processor and aren't a hard technical ceiling here;
        // config.maxNumPatches (1024, pre-downsample patch count) governs
        // this tiled path but is itself just the processor's own default
        // tuning value, not a model architecture limit — nothing in this
        // Swift port enforces it as a hard boundary either.
        //
        // Second pass (current): favors preserving the aspect ratio over
        // hitting a specific token count. Scales BOTH axes down together
        // by the SAME factor (not alternating single-axis decrements —
        // that degenerates to a square once the axes converge, which is
        // exactly the distortion being avoided here) until the combined
        // token budget fits, floored at minTiles. tokenCap is deliberately
        // several times config.maxNumPatches's implied budget — chosen
        // empirically as a conservative middle ground: high enough to
        // keep a meaningfully non-square tile grid (preserving the
        // photo's real proportions), low enough to stay far from the
        // token counts that reproducibly crashed generate() before any
        // fix. Needs on-device verification per model/config combination
        // if this port is reused elsewhere.
        ...
        let tokenCap = 4096
        let originalNumTilesH = numTilesH
        let originalNumTilesW = numTilesW
        var scale = 1.0
        while true {
            let candidateH = max(config.minTiles, Int((Double(originalNumTilesH) * scale).rounded()))
            let candidateW = max(config.minTiles, Int((Double(originalNumTilesW) * scale).rounded()))
            let tokens = (candidateH * patchesPerTileH0 / downsampleFactor0)
                * (candidateW * patchesPerTileW0 / downsampleFactor0)
            if tokens <= tokenCap || (candidateH <= config.minTiles && candidateW <= config.minTiles) {
                numTilesH = candidateH
                numTilesW = candidateW
                break
            }
            scale -= 0.05
        }
